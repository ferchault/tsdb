...
class Stage(object):
    ...
    def get_folders(self, path, label=None):
        ...
        if path != self._get_folders_lastpath:
            self._get_folders_paths = [
                _ for _ in glob.glob("%s/*-*" % path) if os.path.isdir(_)
            ]

        ret = sorted(
            [
                _
                for _ in self._get_folders_paths
                if _.startswith("%s/%s-" % (path.rstrip("/"), label))
            ]
        )
        return ret
            ...
            try:
                if not filecmp.cmp(lastfile, "%s/inp.xyz" % thisrun):
                    self._verbose_message(
                        "run.xyz from last run is not inp.xyz of this run"
                    )
                    return False
            except FileNotFoundError:
                self._verbose_message(
                    "Previous run.xyz / current inp.xyz cannot be found."
                )
                return False
    ...
    def _xyz_continuation(self, folders, req):
        ...
        if not filecmp.cmp(lastfile, "%s/inp.xyz" % folders[0]):
            self._verbose_message(
                "Continuation: last run.xyz or current inp.xyz not found."
            )
            return False
        return True
    ...
    def _do_init_nf_scale(self, folder, path, req, scale):
        ...
        with open("%s/run.inp" % folder, "w") as runfile:
            runfile.write(
                orca_sd_input % (self.get_solvent(), self._charge, self._spin)
            )
...
class TS_Stage(Stage):
    ...
    def _do_check_input(self, folders, req):
        ...
        testA = self._check_orca_input(
            lines, folders, "! MP2 6-311G(d) OptTS NoFrozenCore ExtremeSCF"
        )
        testB = self._check_orca_input(
            lines, folders, "! MP2 6-311G(d) OptTS NoFrozenCore"
        )
...
class NF_Stage(Stage):
    ...
    def validate_e2_ts(logfile):
        ...
        def check_displacements_e2(coords, displacements):
            ...
            bond_LG_new = (coords[0] + displacements[0]) - (
                coords[2] + displacements[2]
            )
            bond_H = coords[1] - coords[4]
            bond_H_new = (coords[1] + displacements[1]) - (coords[4] + displacements[4])
            bond_Nu = coords[4] - coords[3]
            bond_Nu_new = (coords[4] + displacements[4]) - (
                coords[3] + displacements[3]
            )
        ...
        return (
            check_neg_freqs(data.vibfreqs)
            and check_displacements_e2(data.atomcoords[-1], data.vibdisps[0])
            and check_CX_vs_XH(data.atomnos, data.atomcoords[-1])
        )
    ...
    def validate_sn2_ts(logfile):
        ...
        def calc_volume(coords):
            """Calculate the convex volume generated by the reacting centre and the three substituents"""
            mask = select_closest(distzero(coords))
            volume, area = get_sssa_components(coords[mask, :])
        
            return (volume, area)
        
        def get_sssa_components(coordinates):
            """Function colled by calc volume"""
            hull = scipy.spatial.ConvexHull(coordinates, qhull_options="QJ")
        
            return hull.volume, hull.area
        
        def distzero(coords):
            """Calculates the distance of all the atoms from the reactinc centre (atom 0)"""
            dist = []
            atom0 = coords[0, :]
            for i in coords[1:, :]:
                d = np.sqrt(
                    (atom0[0] - i[0]) ** 2 + (atom0[1] - i[1]) ** 2 + (atom0[2] - i[2]) ** 2
                )
                dist.append(d)
        
            return np.array(dist)

        def select_closest(dist):
            """Identify the three atoms closest to the reacting centre that are not the LG or the Nu"""
            distatoms = np.argsort(dist) + 1
            distatoms = distatoms[(distatoms != 2) & (distatoms != 3)][0:3]
            return np.append(distatoms, 0)
        
        def angle(coords):
            """Calculates the X---C---Y angle"""
            xc = coords[1, :] - coords[0, :]
            yc = coords[2, :] - coords[0, :]
        
            cosine = np.dot(xc, yc) / (np.linalg.norm(xc) * np.linalg.norm(yc))
            angle = np.arccos(cosine)
            return np.degrees(angle)
        
        
        def check_structure(coords):
            """ X--C--Y must be linear (threshold 145) """
            return angle(coords[[0, 2, 3], :]) > 145
        
        
        def check_neg_freqs(freqs):
            """There must be 1 and only 1 imaginary frequency (threshold -50)"""
            return np.amin(freqs) < -50 and len(freqs[freqs < 0]) == 1
        
        
        def check_displacements_sn2(coords, displacements):
            """The imaginary frequency must be an asymmetric stretching of X--C--Y"""
            # get bonds
            bond_LG = coords[0] - coords[2]
        ...
        def check_displ_magnitude_sn2(displacements, atom_types):
            """OPTIONAL TEST: X, C and Y must be the among the most displaced by the vibration"""
            norm_displ = np.linalg.norm(displacements, axis=1) * atom_types
            halfpoint = len(atom_types) // 2
            largedispl = np.argsort(norm_displ)[-halfpoint:]
            return ((3 in largedispl) + (2 in largedispl) + (0 in largedispl)) >= 2
        
        
        def check_volume_change(ts, displaced):
            """OPTIONAL TEST: the pentacoordinated reacting carbon should be planar in the TS and not planar after the displacement"""
            return (calc_volume(displaced)[0] - calc_volume(ts)[0]) > 0
        
        
        def check_movement(ts, displaced):
            """X and Y should be the ones whose distance from C changes the most with the diplacement"""
            deltadispl = np.abs(distzero(ts) - distzero(displaced))
            sort_idx_displ = (
                np.argsort(deltadispl)[::-1] + 1
            )  # the +1 is needed because the distances are referred to atom 0, thus the indexes are scaled down by one
        ...
        mandatory = (                                              
            check_structure(ts)                                    
            * check_structure(displaced)                           
            * check_neg_freqs(data.vibfreqs)                       
            * check_displacements_sn2(ts, data.vibdisps[0])        
            * check_movement(ts, displaced)                        
        )                                                          
                                                                    
        if mandatory:                                              
            return check_displ_magnitude_sn2(                      
                data.vibdisps[0], data.atomnos                     
            ) or check_volume_change(ts, displaced)                
        else:                                                      
            return False                                           
    ...
    def _do_init(self, folder, path, req):
        ...
        with open("%s/run.inp" % folder, "w") as runfile:
            runfile.write(
                orca_nf_input % (self.get_solvent(), self._charge, self._spin)
            )
    ...
    def _do_check_input(self, folders, req):
        lines = open("%s/run.inp" % folders[-1], errors="replace").readlines()
        testA = self._check_orca_input(
            lines, folders, "! MP2 6-311G(d) OptTS NoFrozenCore ExtremeSCF NumFreq"
        )
        testB = self._check_orca_input(
            lines, folders, "! MP2 6-311G(d) NoFrozenCore ExtremeSCF NumFreq"
        )
        testC = self._check_orca_input(
            lines, folders, "! MP2 6-311G(d) NoFrozenCore NumFreq"
        )
...
class LS_Stage(Stage):
    def _do_check_input(self, folders, req):
        lines = open("%s/run.inp" % folders[-1], errors="replace").readlines()
        if not self._check_orca_input(
            lines, folders, "! MP2 6-311G(d) Opt NoFrozenCore ExtremeSCF"
        ):
            return False
        return True
...
class SD_Stage(Stage):
    ...
    def _do_check_output(self, folders, req):
        ...
        for line in open("%s/run.log" % folders[-1], errors="replace").readlines():
            ...
            if "THE OPTIMIZATION HAS CONVERGED" in line:
                geoopt_complete = True
            if orca_ok and geoopt_complete:
                break
        else:
    ...
    def _do_init_sp(self, folder, path, req):
        ...
        with open("%s/run.inp" % folder, "w") as runfile:
            runfile.write(
                orca_go_input % (self.get_solvent(), self._charge, self._spin)
            )
...
class Split_Stage(Stage):
    ...
    def _identify_type(folder, rxn):
        ...
        # check bond lengths
        if rxn == "sn2":
            if np.linalg.norm(coords[0] - coords[3]) > np.linalg.norm(
                coords[0] - coords[2]
            ):
                coords = np.delete(coords, 3, 0)
                atoms = np.delete(atoms, 3, 0)
                side = "reactant"
            elif np.linalg.norm(coords[0] - coords[3]) < np.linalg.norm(
                coords[0] - coords[2]
            ):
    ...
    def _do_init(self, folder, path, req):
        ...
        identified = [
            Split_Stage._identify_type(_[-1], self._options["rxn"]) for _ in reqfolders
        ]
...
class Workflow(object):
    ...
    def read_path(self, path, verbose=False):
        ...
        progress[StageStates.PENDING] = list(
            set(self._labels) - set(sum(progress.values(), []))
        )
        if verbose:
    ...
    def __init__(self, implicit="gasphase"):
        ...
        self.add_stage(
            NF_Stage(
                ...
                spin=1,
                implicit=implicit,
                validation=NF_Stage.validate_e2_ts,
        ...
        self.add_stage(
            SD_Stage(label="sdA", charge=-1, spin=1, implicit=implicit, forward=True),
            ["nf"],
        )
        self.add_stage(
            SD_Stage(label="sdB", charge=-1, spin=1, implicit=implicit, forward=False),
            ["nf"],
        )
        self.add_stage(
            Split_Stage(label="sp", charge=-1, spin=1, implicit=implicit, rxn="e2"),
            ["sdA", "sdB"],
        )
        for label in "RP":
            self.add_stage(
                SD_Stage(
                    ...
                    spin=1,
                    implicit=implicit,
                    validation=SD_Stage.validate_single_molecule,
                ),
                ["sp"],
            )
            self.add_stage(
                NF_Stage(
                    ...
                    validation=NF_Stage.validate_ignore,
                ),
                ["go%s" % label],
            )
    ...
    def __init__(self, implicit="gasphase"):
        ...
        self.add_stage(
            NF_Stage(
                ...
                spin=1,
                implicit=implicit,
                validation=NF_Stage.validate_sn2_ts,
                continuation_check=False,
            ),
            ["ts"],
        )
        ...
        self.add_stage(
            Split_Stage(label="sp", charge=-1, spin=1, implicit=implicit, rxn="sn2"),
            ["sdA", "sdB"],
        )
        for label in "RP":
            self.add_stage(
